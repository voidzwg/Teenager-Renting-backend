...
def register(request):  # 继承请求类
    if request.method == 'POST':  # 判断请求方式是否为 POST（要求POST方式）
        ...
        if username is None or password_2 is None or password_1 is None or name is None:
            return JsonResponse({'errno': 1006, 'msg': "参数不全"})
...
def login(request):
    if request.method == 'POST':
        ...
        if user.password == password:  # 判断请求的密码是否与数据库存储的密码相同
            ...
            return JsonResponse({'errno': 0, 'msg': "登录成功",'id':user.id})
        else:
